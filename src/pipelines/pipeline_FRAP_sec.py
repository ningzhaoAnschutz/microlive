...
def find_frap_coordinates(image_TXY, frap_time, stable_FRAP_channel, min_diameter):
    ...
    image_before_frap = image_TXY[frap_time - 1]#, z, :, :, channel]
    image_after_frap = image_TXY[frap_time] #, z, :, :, channel]
...
def segment_image(image_TXY, step_size=5, pretrained_model_segmentation=None, frap_time=None, pixel_dilation_pseudo_cytosol=10,stable_FRAP_channel=0,min_diameter=10):
    ...
    if pretrained_model_segmentation is not None:
        model_nucleus = models.CellposeModel(gpu=False, pretrained_model=pretrained_model_segmentation)
        model_nucleus = denoise.CellposeDenoiseModel(gpu=False, pretrained_model=pretrained_model_segmentation,
                                     restore_type="denoise_nuclei")
    else:
        model_nucleus = denoise.CellposeDenoiseModel(gpu=False, model_type="nuclei",
                                     restore_type="denoise_nuclei")
...
def create_image_arrays(list_concatenated_images, selected_image=0, FRAP_channel_to_quantify=0,pretrained_model_segmentation=None,frap_time=None, starting_changing_frame=40, step_size_increase=5,min_diameter=10):
    ...
    masks_TXY, background_mask, pseudo_cytosol_masks_TXY = segment_image(image_TXY_8bit, step_size=5, pretrained_model_segmentation=pretrained_model_segmentation,frap_time=frap_time,stable_FRAP_channel=FRAP_channel_to_quantify,min_diameter=min_diameter)
...
def concatenate_images(list_images, list_names, convert_to_8bit=False,list_time_intervals=None):
    """
    Concatenates sets of three images based on scene and series order, ensuring consistency across scenes and series order.
    ...
    for i, cell_id in enumerate (list_unique_scenes):
        ...
        try:
            ...
            # if there is no 'Pre' in the name, then continue to the next cell_id
            continue
        ...
        try:
            ...
            # if there is no 'Pb1' in the name, then continue to the next cell_id
            continue
        # find the index of list_names that contain the cell_id and string 'Pb2'   
        try:
            index_pb2 = [i for i, name in enumerate(list_names) if cell_id in name and 'Pb2' in name][0]
        except:
            # if there is no 'Pb2' in the name, then continue to the next cell_id
            continue
        image = np.concatenate([list_images[index_pre],list_images[index_pb1],list_images[index_pb2]],axis=0)
        if list_time_intervals is not None:
            list_time = [list_time_intervals[index_pre],list_time_intervals[index_pb1],list_time_intervals[index_pb2]]
        else:
            list_time = None
        if convert_to_8bit:
            # number of channels
...
#     mask_intensity_background=None,
#     use_frap_time_for_roi_detection=False,
#     max_roi_displacement_px=5
# ):
#     """
#     Function to find the FRAP region of interest (ROI) in the image.

#     If use_frap_time_for_roi_detection is True, the function detects the FRAP ROI based on the change
#     in intensity between the frame before FRAP and the frame after FRAP, and uses the detected coordinates
#     for all frames.

#     Args:
#         image_TZXYC_masked: The masked image stack.
#         image_TZXYC: The original image stack.
#         masks_TXY: Masks for each frame.
#         frap_time: The frame number when FRAP occurs.
#         min_diameter: Minimum diameter of the ROI.
#         FRAP_channel_to_quantify: The channel to quantify.
#         stable_FRAP_channel: The channel used for stable FRAP detection.
#         show_binary_plot: Whether to show binary plots.
#         list_selected_frames: List of selected frames for plotting.
#         list_selected_frame_values_real_time: List of real-time values for selected frames.
#         mask_intensity_background: Background mask for intensity calculation.
#         use_frap_time_for_roi_detection: Whether to use FRAP time for ROI detection.

#     Returns:
#         mean_roi_frap, mean_roi_frap_normalized, coordinates_roi, df_selected_trajectory
#     """
#     num_frames = image_TZXYC.shape[0]
#     if use_frap_time_for_roi_detection:
#         # Use the difference between frames at frap_time and frap_time - 1
#         z = 0  # Use first Z-slice,  as FRAP data only has one z slice.
#         channel = stable_FRAP_channel  # The channel to use for detection
#         if frap_time <= 0:
#             raise ValueError("frap_time must be greater than 0 to compare frames.")
#         # Extract frames before and after FRAP
#         #image_before_frap = image_TZXYC_masked[frap_time - 1, z, :, :, channel]
#         #image_before_frap = image_TZXYC_masked[0, z, :, :, channel]
#         # calculate the mean intensity of the image before frap
#         image_before_frap = np.mean(image_TZXYC_masked[0:frap_time-1, z, :, :, channel])
#         image_after_frap = image_TZXYC_masked[frap_time+1, z, :, :, channel]
#         # average of the 3 frames after frap
#         #image_after_frap = np.mean(image_TZXYC_masked[frap_time:frap_time+3, z, :, :, channel])
#         # x_coord_frap_roi, y_coord_frap_roi  = find_frap_coordinates(image_TZXYC_masked, frap_time, stable_FRAP_channel, min_diameter)
#         # if x_coord_frap_roi is not None:
#         # Compute the difference image
#         diff_image = image_after_frap.astype(np.float32) - image_before_frap.astype(np.float32)
#         neg_diff_image = -diff_image  # Regions with decreased intensity are positive
...
#             centroid = best_region.centroid  # (row, col)
#             # Create df_selected_trajectory with centroid repeated for all frames
#             df_selected_trajectory = pd.DataFrame({
#                 'frame': range(num_frames),
#                 'x': [centroid[1]] * num_frames,  # x coordinate
#                 'y': [centroid[0]] * num_frames,  # y coordinate
#             })
#             # Build coordinates_roi array
#             coordinates_roi = df_selected_trajectory[['x', 'y']].values
#             # Constrain ROI movement between consecutive frames
#             if max_roi_displacement_px is not None:
#                 max_disp_sq = max_roi_displacement_px ** 2
#                 for i in range(1, coordinates_roi.shape[0]):
#                     dx = coordinates_roi[i, 0] - coordinates_roi[i - 1, 0]
#                     dy = coordinates_roi[i, 1] - coordinates_roi[i - 1, 1]
#                     if dx * dx + dy * dy > max_disp_sq:
#                         coordinates_roi[i] = coordinates_roi[i - 1]
#             # Compute mean_roi_frap
...
#             )
#             # Normalize mean_roi_frap
#             mean_roi_frap_normalized = mean_roi_frap / np.mean(mean_roi_frap[:frap_time])
#         else:
#             # No region detected
#             mean_roi_frap = mean_roi_frap_normalized = coordinates_roi = df_selected_trajectory = None
#     else:
...
#                     if show_binary_plot:
#                         plot_images_frap(
#                             np.expand_dims(np.expand_dims(image_TXY_binarized[frap_time::1, :, :], axis=1), axis=-1),
...
#                     # Calculate the minimal distance to any point in centroids_df_before
...
#                     # Filter rows where the minimum distance is greater than or equal to the threshold
...
#                         list_coordinates_roi = []
#                         list_df_selected_trajectory = []
#                         list_diff_roi_frap = []
...
#                             )
#                             mean_roi_frap_normalized = mean_roi_frap / np.mean(mean_roi_frap[:frap_time])
#                             diff_roi_frap = np.diff(mean_roi_frap_normalized)
#                             list_mean_roi_frap.append(mean_roi_frap)
#                             list_mean_roi_frap_normalized.append(mean_roi_frap_normalized)
#                             list_coordinates_roi.append(coordinates_roi)
#                             list_df_selected_trajectory.append(df_selected_trajectory)
#                             list_diff_roi_frap.append(np.min(diff_roi_frap[0:frap_time + 5]))
#                 except Exception as e:
#                     print(f"Exception occurred: {e}")
#                     continue
#                 if np.min(list_diff_roi_frap) < -0.3:
#                     break_condition_met = True
#                     break
#             if break_condition_met:
#                 break
#         if break_condition_met:
#             index_selected = np.argmin(list_diff_roi_frap)
#             mean_roi_frap = list_mean_roi_frap[index_selected]
#             mean_roi_frap_normalized = list_mean_roi_frap_normalized[index_selected]
#             coordinates_roi = list_coordinates_roi[index_selected]
#             df_selected_trajectory = list_df_selected_trajectory[index_selected]
#         else:
#             mean_roi_frap = mean_roi_frap_normalized = coordinates_roi = df_selected_trajectory = None
...
def detect_roi_by_difference(
    ...
    """
    Detect ROI by difference of pre- and post-FRAP images.
    Returns coordinates_roi (shape [num_frames,2]) and df_selected_trajectory (frame,x,y).
    """


    z = 0
    channel = stable_FRAP_channel
    num_frames = image_TZXYC.shape[0]
    # Pre- and post-FRAP frames
    if frap_time <= 0 or frap_time >= image_TZXYC.shape[0]-1:
        return None, None
    image_before = image_TZXYC_masked[frap_time-1, z, :, :, channel]
    image_after = image_TZXYC_masked[frap_time+1, z, :, :, channel]
    # Difference and filter
    diff = image_after.astype(np.float32) - image_before.astype(np.float32)
    diff = gaussian_filter(diff, sigma=1)
    neg_diff = -diff
    # Otsu threshold
    try:
        threshold = threshold_otsu(neg_diff)
    except Exception:
        return None, None
    binary = neg_diff > threshold
    # Morphological ops
    binary = binary_opening(binary, footprint=np.ones((3, 3)))
    binary = binary_closing(binary, footprint=np.ones((3, 3)))
    # Label and regionprops
    lbl = label(binary)
    props = regionprops(lbl)
    expected_area = np.pi * (min_diameter / 2) ** 2
    area_tol = expected_area * 0.5
    # Find region with area within ±50% and best circularity
    best_region = None
    best_circ = 0
    for prop in props:
        area = prop.area
        ...
        if abs(area - expected_area) <= area_tol and circ > best_circ:
            best_region = prop
            best_circ = circ
    center_yx = None
    radius = min_diameter / 2
    if best_region is not None:
        center_yx = best_region.centroid  # (y, x)
    else:
        # Try Hough Circle Transform
        from skimage.feature import canny
        from skimage.draw import circle_perimeter
        edges = canny(neg_diff.astype(np.float32), sigma=2)
        hough_r = int(np.round(min_diameter/2))
        hough_res = hough_circle(edges, [hough_r])
        accums, cx, cy, radii = hough_circle_peaks(hough_res, [hough_r], total_num_peaks=1)
        if len(cx) > 0:
            center_yx = (cy[0], cx[0])
    if center_yx is None:
        return None, None
    # Build coordinates_roi array
    coordinates_roi = np.tile([center_yx[1], center_yx[0]], (num_frames, 1))
    # Vectorized displacement clamp
    if max_roi_displacement_px is not None:
